...
public class fragment_edit_tovariuslugi_home extends Fragment {
        ...
        public fragment_edit_tovariuslugi_home() {}
   ...
   View rootView = inflater.inflate(R.layout.fragment_edit_tovariuslugi_home, container, false);
...
//=================КОНЕЦ СЕКЦИИ ЗАКРЫТИЯ МЕНЮ ПРИ НАЖАТИИ НА ЭКРАН================= 
 ...
 //=================ВЫСТАВЛЕНИЕ РЕЖИМА ОТОБРАЖЕНИЯ ТЕЛЕФОН-ПЛАНШЕТ=================
    ...
    try {
    SQLiteDatabase db=DB.getReadableDatabase();   
    Cursor cursor = db.rawQuery("SELECT * FROM KONTRAGENTI WHERE ID = " + ID, null); 
    cursor.moveToNext(); //без этого exception 
    ...
    //=================КОНЕЦ СЕКЦИЯ ЗАПОЛНЕНИЯ СТРАНИЦЫ ДАННЫМИ ИЗ БД=================
        ...
        DBsaveFirmKontragent.setOnClickListener(new View.OnClickListener() {
           ...
           try {
            ...
            DB.ChangeKontragent(ID,FullNameStr,SokrNameStr,INNStr,KPPStr,OGRNStr,BankNameStr,BankBIKStr,BankKSStr,BankRSStr,RukDolzhnStr,VliceStr,FIOrukStr,UrAddrStr,FaktAddrStr,PostAddrStr, PhoneStr,MobileStr,EmailStr,SiteStr,OtvetstvennijStr);
             
                  
             Toast.makeText(getActivity(),"Данные контрагента успешно записаны в базу",Toast.LENGTH_LONG).show();  
             
             MainActivity rootActivity = (MainActivity)getActivity(); 
             rootActivity.closekontragenthome();  
  ...
  Button DBdelKONTRAGENT=(Button)rootView.findViewById(R.id.butondelkontragent);
            ...
            ad.setTitle("Удалить контрагента из базы?");
            ...
            ad.setNegativeButton("Не удалять",
                 ...
                 new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int id) {
                     ...
                     Toast.makeText(getActivity(),"Данные контрагента удалены из базы",Toast.LENGTH_LONG).show();
